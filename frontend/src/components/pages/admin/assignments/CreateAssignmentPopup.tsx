...
import CreateAssignmentValidation from "../../../../validation/CreateAssignmentValidation";
import DatePicker from "react-datepicker";
import "react-datepicker/dist/react-datepicker.css";

type Props = {
  isActiveCreateAssignmentPopup: boolean;
  setIsActiveCreateAssignmentPopup: Dispatch<SetStateAction<boolean>>;
};

type TCreateAssignmentType = {
...
const CreateAssignmentPopup = ({ setIsActiveCreateAssignmentPopup }: Props) => {
  ...
  const initialValues: TCreateAssignmentType = {
    assignmentTitle: "",
    assignmentDesc: "",
    subject: "",
    deadline: null,
  ...
  const { values, handleSubmit, handleBlur, setFieldValue, touched, errors } =
    useFormik({
      ...
      onSubmit: () => {
        ...
        if (status !== "pending")
          dispatch(createAssignment(formData)).then(() => {
            setIsActiveCreateAssignmentPopup(false);
    ...
    <>
      <div className="w-[550px] p-4 bg-white rounded-md shadow-sm outline outline-gray-200">
        <header className="flex items-center justify-between gap-4 w-full h-6">
          ...
          {/* Exit Button */}
          <button
            onClick={() => setIsActiveCreateAssignmentPopup(false)}
